...
def _ensure_status2_map():
    for c in [
        ...
        '/content/tpok003/examples/status2_map.json',
...
def _run_nbutest(file1, file2, date_str):
    ...
    settings = {'files': [file1.name, file2.name], 'поточnadата' if False else 'поточнадата': date_str}
    ...
    for p in ['/content/repo/scripts/nbutest.py',
              '/content/tpok003/scripts/nbutest.py',
    ...
    if latest:
        return latest
    raise gr.Error('Итоговый Excel не найден. Проверьте вход и дату.')
